...
public class Fb {
		...
		*/
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				checkPassword(logInPasswordField.getPassword(),tf.getText());
			}
		});
		...
		creatAc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if(! mobOrEmail.equals(reEnteredMobOrEmail)) JOptionPane.showMessageDialog(null, "Email mismatch, please try again","error",JOptionPane.ERROR_MESSAGE);
				if(! checkEmail(mobOrEmail))JOptionPane.showMessageDialog(null, "Account already exist,"
						+ "Enter another email","Error", JOptionPane.ERROR_MESSAGE);
				else{
					firstName = name.getText();
					surName = surname.getText();
					Account account = new Account(firstName, surName, mobOrEmail, 
	...
	private boolean checkEmail(String mobOrEmail) {
		for(String email:accounts.keySet()){
			if(mobOrEmail.equals(email))return false;
		}
